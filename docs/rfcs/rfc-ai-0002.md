...
Rendered page: https://magpie.apache.org/docs/rfcs/rfc-ai-0002/

Source: https://github.com/apache/magpie/blob/main/docs/rfcs/RFC-AI-0002.md
...
[`PRINCIPLES.md` §1](https://github.com/apache/magpie/blob/main/PRINCIPLES.md#1-avoiding-prompt-fatigue) ranks avoiding it
...
- **The key's own cache.** The recommended policy on the signing slot is `cached`: a touch is required and then honoured for 15 seconds on that slot. One touch covers a rebase that replays a dozen signed commits. The cache lives in the key, not in any host software, so no configuration on the host can extend it. The authentication slot stays `off`. (`ykman openpgp keys set-touch sig cached` / `aut off`.) One exception: with ssh signatures (`gpg.format=ssh`) the signature is made by the authentication slot, so that slot carries the `cached` policy and the transport pays the touch too; OpenPGP signing, or an https remote, avoids it.
...
| `claude-code` | floor `min_version ≥ 2.1.202`, install `@latest` | — | none | Agent harness. Unpinned; installed at the latest release, with a hard `min_version` floor enforced by verify (hard-fails below it under Claude Code). |
