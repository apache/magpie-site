...
Rendered page: https://magpie.apache.org/docs/rfcs/rfc-ai-0004/

Source: https://github.com/apache/magpie/blob/main/docs/rfcs/RFC-AI-0004.md
...
3. **Pilot diversity.** Validation runs (per the Apache Magpie [MISSION.md](https://github.com/apache/magpie/blob/main/MISSION.md)) cover at least one frontier-model backend, at least one fully-local inference setup (Ollama / vLLM / equivalent), and at least one Apache-hosted or Apache-aligned endpoint. A framework that only validates against one vendor is a vendor-locked framework that has not noticed yet. The Apache-hosted leg now has a concrete target: **LLMAO** (`llm.apache.org`), the Foundation's sanctioned-inference gateway, went live in September 2026 serving self-hosted models to committers against a personal access token. It does not yet close this milestone — tool use over its Anthropic-compatible path is broken upstream (LiteLLM routes it to vLLM's `/v1/responses` with a `tool_choice` shape vLLM rejects), and Magpie's skills are tool-driven, so plain conversation is all that currently works against it. The ASF defaults for the gateway, including its model list and known limitations, are recorded in [`organizations/ASF/organization.md`](https://github.com/apache/magpie/blob/main/organizations/ASF/organization.md).
...
1. **Approved-LLM gate.** The project's PMC declares the set of LLMs that may receive private content. The list is per-PMC, not per-framework — Apache Magpie's gate-check tool (`tools/privacy-llm/checker/`) enforces the gate but the *policy* is the project's. The reference list defaults to "Claude Code trusted; `*.apache.org` auto-approved; `localhost` for local-inference setups; everything else requires explicit opt-in." The apache.org default is a **rebuttable** one, not a guarantee: it assumes the endpoint runs on infra under ASF governance, and an apache.org host that does not meet that assumption is carved out by name. `llm.apache.org` is the first such carve-out — it serves from rented third-party GPU hardware and its operators state that pilot traffic must be treated as visible to gateway admins, so it is not approved for `<private-list>` or `<security-list>` content even though the domain matches. Each new ASF inference endpoint gets checked against the assumption rather than inheriting the default unexamined.
