...
| [`release-archive-sweep`](https://github.com/apache/magpie/blob/main/skills/release-archive-sweep/SKILL.md) | Agentic Triage | 12 | **experimental** | Scan the distribution area, identify releases past retention, propose the backend-shaped archival step — the `svn mv` sequence under `svnpubsub`, or archiving in ATR (which removes the files from `dist/release` itself) under `atr`. Agent never archives. |
