...
Rendered page: https://magpie.apache.org/docs/release-management/readme/

Source: https://github.com/apache/magpie/blob/main/docs/release-management/README.md
...
process documented in [`process.md`](/docs/release-management/process).
...
plugs in through [`<project-config>/`](https://github.com/apache/magpie/tree/main/projects/_template/)
...
[`release-management-config.md`](https://github.com/apache/magpie/blob/main/projects/_template/release-management-config.md).
...
[`process.md` § Adopter backends](/docs/release-management/process#adopter-backends)
...
![An animated `/magpie-setup config` run for the release-management family: the check failing, the values derived from the repository, one question for the rest, and gitignored files written](/docs-assets/quickstart/wizard/release-management.svg)
...
|---|---|---|
| [`pmc-roster.md`](https://github.com/apache/magpie/blob/main/projects/_template/pmc-roster.md) | Who is binding. Read wherever a vote is counted or a PMC-only action is gated. | `promote`, `vote-tally` |
| [`release-build.md`](https://github.com/apache/magpie/blob/main/projects/_template/release-build.md) | How this project builds and signs artefacts: build command, artefact names, checksum algorithm, signing-key expectations. | `rc-cut`, `verify-rc` |
| [`release-management-config.md`](https://github.com/apache/magpie/blob/main/projects/_template/release-management-config.md) | Vote window and pass rule, distribution backend and paths, announce/vote list addresses, retention rule. | `announce-draft`, `archive-sweep`, `audit-report`, `keys-sync`, `prepare`, `promote`, `rc-cut`, `verify-rc`, `vote-draft`, `vote-tally` |
| [`release-trains.md`](https://github.com/apache/magpie/blob/main/projects/_template/release-trains.md) | Active release branches, release-manager attribution per cut, rotation rosters, security-team roster. | `archive-sweep`, `prepare` |

**Optional.** Each has a documented fallback; absent, the skill still runs.

| File | What it carries | Read by |
|---|---|---|
| [`canned-responses.md`](https://github.com/apache/magpie/blob/main/projects/_template/canned-responses.md) | Reusable reporter-facing reply templates. | `announce-draft`, `vote-draft` |
...
![A prepare run that drafted a planning issue and a version-bump PR for 1.4.0 and filed neither](/docs-assets/quickstart/families/release-management/prepare.svg)
...
![A verify-rc run: five green checks across signatures, checksums, licence headers, NOTICE and prohibited binaries, one warning about a stale version string, and a PASS WITH WARNINGS verdict](/docs-assets/quickstart/families/release-management/verify-rc.svg)
...
![A vote-tally run counting three binding +1s and no -1s, and drafting the RESULT email without sending it](/docs-assets/quickstart/families/release-management/vote-tally.svg)
...
| Skill | PR | Step(s) |
|---|---|---|
| [`release-prepare`](https://github.com/apache/magpie/blob/main/skills/release-prepare/SKILL.md) | #540 | 1, 2, 14 |
| [`release-keys-sync`](https://github.com/apache/magpie/blob/main/skills/release-keys-sync/SKILL.md) | #541 | 3 |
| [`release-rc-cut`](https://github.com/apache/magpie/blob/main/skills/release-rc-cut/SKILL.md) | #543 | 4, 5 |
| [`release-verify-rc`](https://github.com/apache/magpie/blob/main/skills/release-verify-rc/SKILL.md) | #537 | 6 |
| [`release-vote-draft`](https://github.com/apache/magpie/blob/main/skills/release-vote-draft/SKILL.md) | #582 | 7 |
| [`release-vote-tally`](https://github.com/apache/magpie/blob/main/skills/release-vote-tally/SKILL.md) | #533 | 9 |
| [`release-promote`](https://github.com/apache/magpie/blob/main/skills/release-promote/SKILL.md) | #544 | 10 |
| [`release-announce-draft`](https://github.com/apache/magpie/blob/main/skills/release-announce-draft/SKILL.md) | #512 | 11 |
| [`release-archive-sweep`](https://github.com/apache/magpie/blob/main/skills/release-archive-sweep/SKILL.md) | #551 | 12 |
| [`release-audit-report`](https://github.com/apache/magpie/blob/main/skills/release-audit-report/SKILL.md) | #552 | 13 |

Every skill is flagged `experimental` and tracked in
[`docs/modes.md`](/docs/modes).

The family landed as docs first (this README, the 14-step
[`process.md`](/docs/release-management/process), the per-skill [`spec.md`](/docs/release-management/spec), and
the adopter scaffold
[`projects/_template/release-management-config.md`](https://github.com/apache/magpie/blob/main/projects/_template/release-management-config.md))
so the lifecycle, the state-change boundaries, and the adopter
contract are reviewable independently from runtime behaviour.
This pattern matches [Mentoring](/docs/mentoring/readme).
...
See [`MISSION.md` § Initial Goals](https://github.com/apache/magpie/blob/main/MISSION.md#initial-goals)
...
the lifecycle step(s) it owns. Read [`spec.md`](/docs/release-management/spec) for the
per-skill state-change boundary; read [`process.md`](/docs/release-management/process)
for the step it executes against.

| Skill | Mode | Steps owned | Status | Purpose |
|---|---|---|---|---|
| [`release-prepare`](https://github.com/apache/magpie/blob/main/skills/release-prepare/SKILL.md) | Agentic Drafting | 1, 2, 14 | **experimental** | Open the planning issue, draft the version-bump + changelog + NOTICE/LICENSE PR, then draft the post-release `-SNAPSHOT` bump. |
| [`release-keys-sync`](https://github.com/apache/magpie/blob/main/skills/release-keys-sync/SKILL.md) | Agentic Drafting | 3 | **experimental** | Draft the `KEYS` diff for a Release Manager cutting their first release for the project. Agent never holds the private key. |
| [`release-rc-cut`](https://github.com/apache/magpie/blob/main/skills/release-rc-cut/SKILL.md) | Agentic Drafting | 4, 5 | **experimental** | Emit the paste-ready command sequence, signed tag, build, detached signatures, checksums, `svn` import to `dist/dev/<project>/`. Agent never signs and never imports. |
| [`release-verify-rc`](https://github.com/apache/magpie/blob/main/skills/release-verify-rc/SKILL.md) | Agentic Triage / Agentic Pairing | 6 | **experimental** | Read-only pre-flight: signatures against the project's `KEYS`, checksums, license headers (Apache RAT), NOTICE/LICENSE presence, no prohibited binaries, version-string consistency. Voters can run it in their own dev loop before posting `+1`. |
| [`release-vote-draft`](https://github.com/apache/magpie/blob/main/skills/release-vote-draft/SKILL.md) | Agentic Drafting | 7 | **experimental** | Draft the `[VOTE]` email body to `dev@<project>`. Agent never sends. |
| [`release-vote-tally`](https://github.com/apache/magpie/blob/main/skills/release-vote-tally/SKILL.md) | Agentic Triage | 9 | **experimental** | Parse the vote thread, classify each reply (+1 / 0 / -1) binding vs non-binding against the PMC roster, propose `[RESULT] [VOTE]`. Conservative on ambiguous votes, refuses to count, flags `AMBIGUOUS, needs RM call`. |
| [`release-promote`](https://github.com/apache/magpie/blob/main/skills/release-promote/SKILL.md) | Agentic Drafting | 10 | **experimental** | Emit the paste-ready `svn mv dist/dev → dist/release` command set plus commit message. Agent never moves; the human commit is the act of release. |
| [`release-announce-draft`](https://github.com/apache/magpie/blob/main/skills/release-announce-draft/SKILL.md) | Agentic Drafting | 11 | **experimental** | Draft the `[ANNOUNCE]` email body to `announce@apache.org` and the site-bump PR (download page, release notes, version banner). Agent never sends mail and never merges the site PR. |
| [`release-archive-sweep`](https://github.com/apache/magpie/blob/main/skills/release-archive-sweep/SKILL.md) | Agentic Triage | 12 | **experimental** | Scan `dist/release/<project>/`, identify releases past retention, propose the `svn mv` sequence to `archive.apache.org`. Agent never moves. |
| [`release-audit-report`](https://github.com/apache/magpie/blob/main/skills/release-audit-report/SKILL.md) | Agentic Triage (dashboard) | 13 | **experimental** | Read-only structured report per release, RM, voters with binding flags, artefacts with sigs and checksums, promotion revision, `[ANNOUNCE]` archive URL. Output appended to the project's audit log. |
...
- **The agent never holds, invokes, or proxies the Release
  ...
  [`security-cve-allocate`](https://github.com/apache/magpie/blob/main/skills/security-cve-allocate/SKILL.md)
  (Vulnogram URL + paste-ready JSON, human submits) and satisfies
  [RFC-AI-0004 Principle 1](/docs/rfcs/rfc-ai-0004#principle-1--human-in-the-loop-on-every-state-change).
...
- [**`svn-release-runbook.md`**](/docs/release-management/svn-release-runbook), the
  ...
  companion to [`release-rc-cut`](https://github.com/apache/magpie/blob/main/skills/release-rc-cut/SKILL.md).
- [**`atr-release-runbook.md`**](/docs/release-management/atr-release-runbook), the
  ...
  [MISSION § first-release commitment](https://github.com/apache/magpie/blob/main/MISSION.md#initial-goals).
- [**`manual-release-process.md`**](/docs/release-management/manual-release-process), the
...
- [`<project-config>/release-trains.md`](https://github.com/apache/magpie/blob/main/projects/_template/release-trains.md)
...
introduced. See [`docs/modes.md`](/docs/modes) for the
...
[`docs/modes.md` § Meta](/docs/modes#meta)
...
- [`process.md`](/docs/release-management/process), [`spec.md`](/docs/release-management/spec), within this
  family.
- [`MISSION.md` § Initial Goals](https://github.com/apache/magpie/blob/main/MISSION.md#initial-goals),
  the standard-release commitment this family operationalises.
- [`docs/modes.md`](/docs/modes), the mode taxonomy each skill
  in this family inhabits.
- [`docs/security/README.md`](/docs/security/readme), the
  precedent for a multi-skill ASF-process family with shared
  state-change-boundary discipline.
- [`docs/mentoring/README.md`](/docs/mentoring/readme), the
