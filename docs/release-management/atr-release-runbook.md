...
Rendered page: https://magpie.apache.org/docs/release-management/atr-release-runbook/

Source: https://github.com/apache/magpie/blob/main/docs/release-management/atr-release-runbook.md
...
[`svn-release-runbook.md`](/docs/release-management/svn-release-runbook). Both describe the
**same [14-step lifecycle](/docs/release-management/process)** and are driven by the **same
[release-management skills](/docs/release-management/readme)** — they differ only in the
...
| Run checks | RM/voters run [`release-verify-rc`](https://github.com/apache/magpie/blob/main/skills/release-verify-rc/SKILL.md) locally | **Compose** checks run automatically in ATR + local verify |
...
`atr` is a value of the [`release_dist_backend`](https://github.com/apache/magpie/blob/main/projects/_template/release-management-config.md#backends)
...
> the [`svnpubsub` runbook](/docs/release-management/svn-release-runbook) remains the
...
> [`manual-release-process.md`](/docs/release-management/manual-release-process) for what the
...
[14-step lifecycle](/docs/release-management/process) — and onto the release skills that
own each step — like this:

| ATR phase | Lifecycle steps | Owning skill(s) | What happens |
|---|---|---|---|
| *(pre-phase)* | 1 Plan · 2 Changelog/NOTICE/LICENSE | [`release-prepare`](https://github.com/apache/magpie/blob/main/skills/release-prepare/SKILL.md) | Planning issue + version-bump PR. Off-platform; unchanged. |
| *(pre-phase)* | 3 `KEYS` | [`release-keys-sync`](https://github.com/apache/magpie/blob/main/skills/release-keys-sync/SKILL.md) | RM's public key added to the committee's `KEYS` **in ATR**. |
| **Compose** | 4 Cut RC · 5 Stage · 6 Verify | [`release-rc-cut`](https://github.com/apache/magpie/blob/main/skills/release-rc-cut/SKILL.md), [`release-verify-rc`](https://github.com/apache/magpie/blob/main/skills/release-verify-rc/SKILL.md) | Build the source artefact, sign it, upload it to ATR as a draft; ATR runs signature/checksum/license/notice/source-header checks automatically. |
| **Vote** | 7 `[VOTE]` · 8 Window · 9 Tally | [`release-vote-draft`](https://github.com/apache/magpie/blob/main/skills/release-vote-draft/SKILL.md), [`release-vote-tally`](https://github.com/apache/magpie/blob/main/skills/release-vote-tally/SKILL.md) | ATR sends the `[VOTE]` to `dev@` and tabulates replies over ≥72h. The skills draft the body and cross-check the tally against the PMC roster. |
| **Finish** | 10 Promote · 11 Announce | [`release-promote`](https://github.com/apache/magpie/blob/main/skills/release-promote/SKILL.md), [`release-announce-draft`](https://github.com/apache/magpie/blob/main/skills/release-announce-draft/SKILL.md) | ATR strips `-rcN`, rearranges the tree, publishes to `dist.apache.org`, and assists the `[ANNOUNCE]`. Records downstream distributions (PyPI, Maven Central). |
| *(post-phase)* | 12 Archive · 13 Audit · 14 Post-bump | [`release-archive-sweep`](https://github.com/apache/magpie/blob/main/skills/release-archive-sweep/SKILL.md), [`release-audit-report`](https://github.com/apache/magpie/blob/main/skills/release-audit-report/SKILL.md), [`release-prepare`](https://github.com/apache/magpie/blob/main/skills/release-prepare/SKILL.md) | Archiving in ATR, audit-log record, `-SNAPSHOT`/`.dev` bump. |
...
The two non-negotiable [spec](/docs/release-management/spec) boundaries hold identically on
...
- **The agent never holds, invokes, or proxies the RM's signing
  ...
  ([spec § Boundary 1](/docs/release-management/spec#boundary-1-agent-never-holds-the-rms-signing-key)).
- **The agent never publishes the release.** *Composing* a draft and
  *starting* a vote are RM actions in ATR; *finishing* (publishing) is
  ...
  clicks/commits.
  ([spec § Boundary 2](/docs/release-management/spec#boundary-2-agent-never-publishes-the-release)).
...
- **Your OpenPGP key is registered in ATR** for the Magpie committee
  ...
  [`release-keys-sync`](https://github.com/apache/magpie/blob/main/skills/release-keys-sync/SKILL.md)
  (Step 3) first.
- **The prep PR is merged** — version strings, `CHANGELOG`,
  `NOTICE`/`LICENSE` reflect `<version>`
  ([`release-prepare`](https://github.com/apache/magpie/blob/main/skills/release-prepare/SKILL.md), Step 2).
...
[`release-keys-sync`](https://github.com/apache/magpie/blob/main/skills/release-keys-sync/SKILL.md) to
...
1. **Build and sign the source artefact locally** — this half is
   unchanged from the [`svnpubsub` runbook Steps 2–5](/docs/release-management/svn-release-runbook#step-2-tag-the-release-candidate).
   [`release-rc-cut`](https://github.com/apache/magpie/blob/main/skills/release-rc-cut/SKILL.md) emits the
   recipe from [`release-build.md`](https://github.com/apache/magpie/blob/main/projects/_template/release-build.md):
...
3. **Let the checks run.** On upload, ATR fires asynchronous checks:
   ...
   **license**, **notice**, and **source-header** (RAT-style) checks.
   ...
   [`release-verify-rc`](https://github.com/apache/magpie/blob/main/skills/release-verify-rc/SKILL.md); you
...
1. **Draft the `[VOTE]` body** with
   [`release-vote-draft`](https://github.com/apache/magpie/blob/main/skills/release-vote-draft/SKILL.md).
   ...
   human-readable page
   ([`manual-release-process.md` § Manual verification](/docs/release-management/manual-release-process#manual-verification--what-a-voter-runs-before-1)
   at the RC tag), the [reproducibility background](/docs/release-management/reproducibility),
...
3. **72-hour window** (Step 8). Minimum per
   ...
   ([`release-management-config.md` § Vote](https://github.com/apache/magpie/blob/main/projects/_template/release-management-config.md#vote)).
   ...
   - **Agentic:** `/magpie-release-management:verify-rc <version>-rcN`
     ...
     `differs`.
   - **Manual:** the linked
     [verification page](/docs/release-management/manual-release-process#manual-verification--what-a-voter-runs-before-1)
   ...
   - **Reply** on the thread using the
     [reply template](/docs/release-management/manual-release-process#reply-template--what-to-put-in-your-vote):
...
4. **Tally** (Step 9). ATR tabulates the replies; cross-check with
   [`release-vote-tally`](https://github.com/apache/magpie/blob/main/skills/release-vote-tally/SKILL.md),
   which classifies each reply binding-vs-non-binding against the
   [`pmc-roster.md`](https://github.com/apache/magpie/blob/main/projects/_template/pmc-roster.md) and
...
1. **Finish the release** in ATR once the vote has passed. Finishing:
   ...
   [`release-promote`](https://github.com/apache/magpie/blob/main/skills/release-promote/SKILL.md) now
...
2. **Announce** (Step 11).
   ...
   [`site-repo.md`](https://github.com/apache/magpie/blob/main/projects/_template/site-repo.md). ATR can
...
- **Archive sweep** (Step 12): follow the retention rule in
  [`release-management-config.md` § Archive](https://github.com/apache/magpie/blob/main/projects/_template/release-management-config.md#archive).
...
- **Post-release version bump** (Step 14) —
  [`release-prepare`](https://github.com/apache/magpie/blob/main/skills/release-prepare/SKILL.md) drafts
...
see [`reproducibility.md`](/docs/release-management/reproducibility#automated-release-signing--asf-specific-optional). See the
...
- [`process.md`](/docs/release-management/process) — the 14-step lifecycle this runbook
...
- [`spec.md`](/docs/release-management/spec) — the state-change boundaries (agent never
  holds the key, never publishes) that hold identically on ATR.
- [`README.md`](/docs/release-management/readme) — the release-management skill family that
  drives every step.
