...
Rendered page: https://magpie.apache.org/docs/skill-sources/readme/

Source: https://github.com/apache/magpie/blob/main/docs/skill-sources/README.md
...
Per [`PRINCIPLES.md` §14](https://github.com/apache/magpie/blob/main/PRINCIPLES.md#14-snapshot-plus-override-never-vendored-copies),
...
[`setup`](https://github.com/apache/magpie/blob/main/skills/setup/SKILL.md) skill — never a git submodule, and
never an unpinned or unverified auto-fetch. The full rationale and threat
model are in [`RFC-AI-0006`](/docs/rfcs/rfc-ai-0006).
...
|---|---|---|---|
| **Discovery** | [`registry.md`](/docs/skill-sources/registry) | in-tree | The framework's index of known sources — curated **and** community. Editorial only; lists a source, never installs it. |
...
model uses (see [`AGENTS.md`](https://github.com/apache/magpie/blob/main/AGENTS.md#configuration-resolution-order)).
...
[registry](/docs/skill-sources/registry) links to the canonical one. Fields reuse the
[install-method](/docs/quick-start/other-install-methods) and lock vocabulary the
...
framework's own [`.apache-magpie.lock`](https://github.com/apache/magpie/blob/main/skills/setup/SKILL.md) carries;
...
[the validator](https://github.com/apache/magpie/tree/main/tools/skill-and-tool-validator/) apply (the `source:`
...
The [`setup`](https://github.com/apache/magpie/blob/main/skills/setup/SKILL.md) skill drives the fetch — the
[`skill-sources`](https://github.com/apache/magpie/blob/main/skills/setup/skill-sources.md) sub-action
...
2. For each trusted source, **fetch + verify** into
   ...
   [install recipes](/docs/quick-start/other-install-methods) verbatim — `git clone
...
(see [`tools/skill-evals/README.md`](https://github.com/apache/magpie/blob/main/tools/skill-evals/README.md)).
...
- **Untrusted stays discovery-only.** The [registry](/docs/skill-sources/registry) and org
...
fetch) is in [`RFC-AI-0006`](/docs/rfcs/rfc-ai-0006#security-model).

## Discovery index

The known sources — framework-curated and community-maintained — are
listed in [`registry.md`](/docs/skill-sources/registry). Listing is editorial discovery
only; it makes no guarantee and triggers no install.

## See also

- [`authoring-a-source.md`](/docs/skill-sources/authoring-a-source) — the source-repo side: how a third-party org publishes skills for adopters to pull.
- [`RFC-AI-0006`](/docs/rfcs/rfc-ai-0006) — the design + trust + threat model.
- [`docs/extending.md`](/docs/extending) — the full extension model (what / where / who).
- [`organizations/README.md`](https://github.com/apache/magpie/blob/main/organizations/README.md) — the organization layer and its `skill-sources.md` curation.
- [`skills/setup/SKILL.md`](https://github.com/apache/magpie/blob/main/skills/setup/SKILL.md) — the adopt/upgrade/verify flow that fetches and pins sources.
- [`docs/adapters/registry.md`](/docs/adapters/registry) — the sibling discovery index for tool adapters and organizations.
