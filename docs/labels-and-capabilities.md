...
[`tools/vendor-neutrality-score`](https://github.com/apache/magpie/tree/main/tools/vendor-neutrality-score/) reads it
and never counts a partial tool as a backend vendor, so a partial foundation
cannot turn a contract green in
[`docs/vendor-neutrality.md`](/docs/vendor-neutrality).
...
|---|---|---|
| [`tools/agent-guard`](https://github.com/apache/magpie/tree/main/tools/agent-guard/) | `substrate:action-guard` | Deterministic pre-execution guard dispatcher (harness-neutral core behind a Claude Code `PreToolUse` hook, an OpenCode `tool.execute.before` plugin, a Kiro `preToolUse` hook, and a Gemini CLI `BeforeTool` hook): blocks `gh`/`git` commands that would ping maintainers, carry a `Co-Authored-By` trailer, mark-ready prematurely, leak security language publicly, or empty a PR via force-push. Extensible — skills contribute guards via `guards.d` |
...
| [`tools/container-gateway`](https://github.com/apache/magpie/tree/main/tools/container-gateway/) | `substrate:sandbox` | Per-project policy proxy for the podman / docker API; label-scoped, mount- and privilege-checked container access from inside the sandbox |
...
| [`tools/osv`](https://github.com/apache/magpie/tree/main/tools/osv/) | `contract:security-cross-ref` | OSV.dev vulnerability cross-reference client (Vendor: OSV.dev): queries vulnerability records, alias cross-references (CVE ↔ GHSA ↔ OSV IDs), affected package versions, and vulnerabilities associated with a public upstream commit via OSV.dev REST API v1 |
...
| [`tools/adversarial-review`](https://github.com/apache/magpie/tree/main/tools/adversarial-review/) | `substrate:review` | Runs other models' CLIs (Codex, Copilot, Gemini, Claude) read-only over a change before its PR is created, and merges their findings into one advisory JSON report |
...
| [`tools/setup-preflight`](https://github.com/apache/magpie/tree/main/tools/setup-preflight/) | `substrate:setup` | Deterministic setup pre-flight: resolves a project's lock, floor and reconciliation state plus one skill's fingerprint into a JSON verdict, so every skill's shared pre-flight block runs a command instead of re-deriving the rules from prose |
...
| [`tools/skill-token-count`](https://github.com/apache/magpie/tree/main/tools/skill-token-count/) | `substrate:framework-dev` + `substrate:analytics` | Reproducible full-file skill token measurements and documentation drift check |
...
| [`tools/reproducible-archive`](https://github.com/apache/magpie/tree/main/tools/reproducible-archive/) | `substrate:release` | `repro-archive` — build, lint and compare reproducible source archives from a git ref (`git archive` + `.gitattributes` `export-ignore`, every reproducible-builds.org archive rule applied); used by `release-rc-cut` Step 2/2b and `release-verify-rc` Step 9 |
...
[`docs/prerequisites.md`](/docs/quick-start/prerequisites) for connection setup.
...
the scheduled [`.github/workflows/labeler.yml`](https://github.com/apache/magpie/blob/main/.github/workflows/labeler.yml)
labels each new PR once, with the `**Capability:**` of every tool directory it
touches, from
[`.github/labeler.yml`](https://github.com/apache/magpie/blob/main/.github/labeler.yml), which
[`tools/dev/generate-labeler-config.py`](https://github.com/apache/magpie/blob/main/tools/dev/generate-labeler-config.py)
