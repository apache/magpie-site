...
Rendered page: https://magpie.apache.org/docs/quick-start/prerequisites/

Source: https://github.com/apache/magpie/blob/main/docs/quick-start/prerequisites.md
...
- **[OpenAI Codex](https://github.com/openai/codex)** reads Magpie's
  ...
  recorded; see [the Codex harness guide](/docs/adapters/codex).
- **[Gemini CLI](https://geminicli.com/)** reads Magpie's canonical
  ...
  **experimental**: Gemini 0.59.0 has native API checks and a Linux adopter pilot,
  ...
  see [the harness guide's known limitations](/docs/adapters/gemini#known-limitations).
- **[Cursor](https://cursor.com/)** (Composer + `cursor-agent` CLI) reads Magpie's
  ...
  and clean-environment isolation; see [the Cursor harness guide](/docs/adapters/cursor).
- **[Goose](https://github.com/block/goose)** is an open-source agent CLI that reads
  ...
  see [the Goose harness guide](/docs/adapters/goose).
- **[Aider](https://aider.chat/)** is an open-source, multi-model terminal pair programming CLI
  ...
  see [the Aider harness guide](/docs/adapters/aider).
- **[GitHub Copilot](https://github.com/features/copilot)** provides a standalone terminal agent (`copilot`)
  ...
  see [the GitHub Copilot harness guide](/docs/adapters/copilot).
...
- an **open-weight model** — hosted on a provider of your choice, or run
  **locally** (Ollama, llama.cpp, vLLM) for sovereign or air-gapped use;
  see [the Local LLM harness guide](/docs/adapters/local-llm).
...
[`docs/setup/secure-agent-setup.md`](/docs/setup/secure-agent-setup) — a layered
...
- **Read** backends: the
  ...
  **PonyMail** MCP (below), or a **local mbox / Maildir archive** for
  ...
  ([`tools/mail-source/mbox`](https://github.com/apache/magpie/blob/main/tools/mail-source/mbox/README.md)).
...
[`tools/ponymail/tool.md`](https://github.com/apache/magpie/blob/main/tools/ponymail/tool.md) for the
...
[**Maildir** draft backend](https://github.com/apache/magpie/tree/main/tools/maildir/) (`contract:mail-create`,
...
[`<project-config>/project.md → Mail sources`](/docs/quick-start/<project-config>/project#mail-sources)),
...
[`tools/ponymail/tool.md → Keeping the checkout current`](https://github.com/apache/magpie/blob/main/tools/ponymail/tool.md#keeping-the-checkout-current)).
...
see
[`<project-config>/release-trains.md`](/docs/quick-start/<project-config>/release-trains#security-team-roster).
...
[`<project-config>/project.md → CVE tooling`](/docs/quick-start/<project-config>/project#cve-tooling).
...
- the adopting project's dev toolchain installed per
  [`<project-config>/fix-workflow.md → Toolchain`](/docs/quick-start/<project-config>/fix-workflow#toolchain);
...
manifest's
[`project_metadata`](/docs/quick-start/<project-config>/project#project-metadata)
...
[`tools/apache-projects/tool.md`](https://github.com/apache/magpie/blob/main/tools/apache-projects/tool.md).
