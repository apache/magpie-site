...
Rendered page: https://magpie.apache.org/docs/setup/secure-agent-setup/

Source: https://github.com/apache/magpie/blob/main/docs/setup/secure-agent-setup.md
...
[`secure-agent-internals.md`](/docs/setup/secure-agent-internals). It is
...
[Codex first-class harness adapter](/docs/adapters/codex) for that
...
The same four skills route Gemini CLI to its [setup lifecycle](/docs/adapters/gemini#setup-isolated-lifecycle).
...
[Codex adapter](/docs/adapters/codex#setup-isolated-lifecycle).
...
The skills are at
[`.claude/skills/magpie-setup/verify.md`](https://github.com/apache/magpie/blob/main/skills/setup/verify.md),
[`.claude/skills/setup-isolated-setup-install/`](https://github.com/apache/magpie/blob/main/skills/setup-isolated-setup-install/SKILL.md),
[`.claude/skills/setup-isolated-setup-verify/`](https://github.com/apache/magpie/blob/main/skills/setup-isolated-setup-verify/SKILL.md),
[`.claude/skills/magpie-setup/upgrade.md`](https://github.com/apache/magpie/blob/main/skills/setup/upgrade.md),
[`.claude/skills/setup-isolated-setup-update/`](https://github.com/apache/magpie/blob/main/skills/setup-isolated-setup-update/SKILL.md),
[`.claude/skills/setup-shared-config-sync/`](https://github.com/apache/magpie/blob/main/skills/setup-shared-config-sync/SKILL.md).
...
[Codex security model](/docs/adapters/codex#security-model).
...
setting in [`pyproject.toml`](https://github.com/apache/magpie/blob/main/pyproject.toml) and the weekly Dependabot
updates in [`.github/dependabot.yml`](https://github.com/apache/magpie/blob/main/.github/dependabot.yml).
...
[`tools/agent-isolation/pinned-versions.toml`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/pinned-versions.toml):
...
[tool prerequisites](https://github.com/apache/magpie/blob/main/tools/skill-token-count/README.md#prerequisites)
...
[`.claude/settings.json`](https://github.com/apache/magpie/blob/main/.claude/settings.json). The full block is
below, annotated.
...
[`sandbox-add-project-root.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/sandbox-add-project-root.sh)
...
- **`dangerouslyDisableSandbox: true` from agent sessions.**
  ...
  [`sandbox-bypass-warn.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/sandbox-bypass-warn.sh)'s
...
[`/magpie-setup verify`](https://github.com/apache/magpie/blob/main/skills/setup/verify.md)
...
The framework ships
[`tools/agent-isolation/sandbox-add-project-root.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/sandbox-add-project-root.sh)
...
- [`setup-isolated-setup-verify`](https://github.com/apache/magpie/blob/main/skills/setup-isolated-setup-verify/SKILL.md)
...
See
[`setup-isolated-setup-install` Step P.0a](https://github.com/apache/magpie/blob/main/skills/setup-isolated-setup-install/step-p-sandbox-allowlists.md#step-p0a--loud-disclosure-before-setting-whole-user-scope).
...
([`tools/agent-isolation/git-hook-dispatcher.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/git-hook-dispatcher.sh))
...
([`tools/agent-isolation/prek-shim.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/prek-shim.sh),
...
[`tools/agent-isolation/agent-iso.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/agent-iso.sh).
...
[`tools/agent-isolation/sandbox-bypass-warn.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/sandbox-bypass-warn.sh)
...
[`.claude/settings.json`](https://github.com/apache/magpie/blob/main/.claude/settings.json) (which is
...
lives in [`tools/agent-guard`](https://github.com/apache/magpie/blob/main/tools/agent-guard/README.md) and
...
- **commit-trailer** — never let a `git commit` carry a
  ...
  [commit-attribution convention](/docs/setup/commit-attribution) is
...
[tool README](https://github.com/apache/magpie/blob/main/tools/agent-guard/README.md) for the contract and
...
[`setup-isolated-setup-doctor`](https://github.com/apache/magpie/blob/main/skills/setup-isolated-setup-doctor/SKILL.md)
...
|---|---|
| `Could not open a connection to your authentication agent` / `agent refused operation` / `ssh-add: error fetching identities` / `Permission denied (publickey)` | [SSH agent / Yubikey unreachable](/docs/setup/sandbox-troubleshooting#ssh-agent--yubikey-appears-unreachable-from-inside-the-sandbox) |
| `Cannot connect to the Docker daemon` / `open /var/run/docker.sock: operation not permitted` / `Cannot connect to Podman` / podman `connect: permission denied` | [Docker / Podman socket denied](/docs/setup/sandbox-troubleshooting#docker--podman-command-fails-with-a-socket-error) |
| `127.0.0.1 … Permission denied` / `Operation not permitted … bind` / `Errno 49 … assign requested address` / `Connection refused … 127.0.0.1` | [Localhost port-bind blocked](/docs/setup/sandbox-troubleshooting#test-cannot-bind-to-a-localhost-port) |
| `/tmp/…: Read-only file system` / `mktemp: failed to create` | [Temp files fail under `/tmp`](/docs/setup/sandbox-troubleshooting#temp-files-fail-with-read-only-file-system-under-tmp) |
| `x509: OSStatus -26276` / `HTTP 401: Requires authentication (https://api.github.com…` | [`gh` ran inside the sandbox](/docs/setup/sandbox-troubleshooting#gh-fails-with-tls-osstatus--26276-or-http-401-inside-the-sandbox) |
| `command not found: prek` / `uv: command not found` (also `uvx`) | [`prek` or `uv` not found inside the sandbox](/docs/setup/sandbox-troubleshooting#prek-or-uv-not-found-or-cannot-write-its-cache-inside-the-sandbox) |
...
- **Pattern set must stay in lock-step with the catalog.** When a
  new entry lands in [`sandbox-troubleshooting.md`](/docs/setup/sandbox-troubleshooting),
...
The framework ships
[`tools/agent-isolation/sandbox-status-line.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/sandbox-status-line.sh)
...
its own helper; see
[`docs/adapters/add-a-harness.md`](/docs/adapters/add-a-harness). The
...
![The Claude Code /sandbox mode panel: three modes — sandbox with auto-allow (selected), sandbox with regular permissions, and no sandbox — with auto-allow explained as commands running in the sandbox automatically and falling back to regular permissions outside it](/docs-assets/sandbox-modes.png)
...
[`.claude/settings.json`](https://github.com/apache/magpie/blob/main/.claude/settings.json) sets
...
[`tools/agent-isolation/claude-term-bg.sh`](https://github.com/apache/magpie/blob/main/tools/agent-isolation/claude-term-bg.sh)
...
[RFC-AI-0002 → Layer 3b](/docs/rfcs/rfc-ai-0002#layer-3b--hardware-key-touch-physical-confirmation-of-signatures-and-remote-access).
...
[Layer 3 — Forced confirmation](/docs/setup/secure-agent-internals), and the
...
[PRINCIPLES.md §1](https://github.com/apache/magpie/blob/main/PRINCIPLES.md) calls a defect. A push is a
...
- `sandbox.network.allowUnixSockets` names gpg-agent's ssh socket
  ...
  [`sandbox-troubleshooting.md` → SSH agent / Yubikey appears unreachable](/docs/setup/sandbox-troubleshooting#ssh-agent--yubikey-appears-unreachable-from-inside-the-sandbox);
- `sandbox.filesystem.allowRead` names the one public-key file git
  ...
  per
  [`sandbox-troubleshooting.md` → Signed commit fails before any touch when git signs with ssh](/docs/setup/sandbox-troubleshooting#signed-commit-fails-before-any-touch-when-git-signs-with-ssh).
...
[`AGENTS.md`](https://github.com/apache/magpie/blob/main/AGENTS.md) → *Commit and PR conventions*. That rule
...
[`sandbox-troubleshooting.md` → Signed commit fails with "cannot exec" of the touch-overlay wrapper](/docs/setup/sandbox-troubleshooting#signed-commit-fails-with-cannot-exec-of-the-touch-overlay-wrapper).
...
The [container gateway](https://github.com/apache/magpie/blob/main/tools/container-gateway/README.md) is a per-project policy proxy that sits in front of the real `podman` / `docker` daemon socket, runs **outside** the sandbox where it can hold that connection, and exposes two policy-checked sockets of its own for the sandboxed CLI to talk to instead.
...
1. Install the pinned tools per [Install commands](#install-commands)
   above.
2. Copy
   [`.claude/settings.json`](https://github.com/apache/magpie/blob/main/.claude/settings.json) from the framework
...
12. **Container gateway wired.** Only meaningful when `podman` or
    ...
    On any ✗, point at
    [`docs/setup/sandbox-troubleshooting.md` → Docker / Podman command fails with a socket error](/docs/setup/sandbox-troubleshooting#docker--podman-command-fails-with-a-socket-error)
...
4. **comdev MCP checkouts.** If you registered the `ponymail`
   ...
   See [`tools/ponymail/tool.md` → Keeping the checkout current](https://github.com/apache/magpie/blob/main/tools/ponymail/tool.md#keeping-the-checkout-current).
...
![Bold red SANDBOX BYPASS banner immediately above the Claude Code permission prompt](/docs-assets/sandbox-bypass-banner.png)
...
![Sandboxed Bash call to `ls ~/Downloads` blocked by the runtime; surfaced as "read ~/Downloads (outside allowed read paths)" with an offer to retry with the sandbox disabled](/docs-assets/sandbox-blocks-read.png)
...
- [`secure-agent-internals.md`](/docs/setup/secure-agent-internals) — the
...
- [`sandbox-troubleshooting.md`](/docs/setup/sandbox-troubleshooting) —
  ...
  inside the sandbox.
- [`AGENTS.md`](https://github.com/apache/magpie/blob/main/AGENTS.md) — placeholder convention used in skill
  files (`<tracker>`, `<upstream>`, `<security-list>`, …).
- [`README.md`](https://github.com/apache/magpie/blob/main/README.md) — framework overview and how the
