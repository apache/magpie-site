...
Rendered page: https://magpie.apache.org/docs/setup/marketplaces/

Source: https://github.com/apache/magpie/blob/main/docs/setup/marketplaces.md
...
> [!TIP]
> Just want it installed? The [quick start](/docs/quick-start) is the
...
[pinned snapshot install](/docs/setup/install-recipes) is the fallback for cases a
...
> once as [`/magpie:setup-isolated-setup-install`](/docs/quick-start#what-happens-next--the-secure-isolation-setup).
> When a project wants every contributor pinned to one committed version,
> add the [pinned snapshot install](/docs/setup/install-recipes) alongside it. The
...
> [`install-recipes.md`](/docs/setup/install-recipes). That path is **harness-neutral**:
...
> [PRINCIPLES §13](https://github.com/apache/magpie/blob/main/PRINCIPLES.md) rules out, and which would leave eleven
...
- **Why the family plugins alias.** `magpie-security` + `security-issue-triage`
  ...
  in [`tools/dev/check-family-plugins.py`](https://github.com/apache/magpie/blob/main/tools/dev/check-family-plugins.py)
...
2. **As a marketplace** — add `apache/magpie` as a plugin marketplace (CLI or
   ...
   root [`marketplace.json`](https://github.com/apache/magpie/blob/main/marketplace.json).
...
4. **Update** with `gemini extensions update magpie`. Gemini has no lifecycle
   hook, so the shipped [`GEMINI.md`](https://github.com/apache/magpie/blob/main/GEMINI.md) reminds you to run
...
TSC), so it reads the root [`plugin.json`](https://github.com/apache/magpie/blob/main/plugin.json). Add Magpie
...
| Agent | Mechanism |
|---|---|
| **Claude Code** | `SessionStart` hook [`hooks/check-upgrade.sh`](https://github.com/apache/magpie/blob/main/hooks/check-upgrade.sh) compares the installed version to a marker in the plugin's persistent data dir and prompts on change. Deterministic. |
| **Codex CLI** | The same [`hooks/check-upgrade.sh`](https://github.com/apache/magpie/blob/main/hooks/check-upgrade.sh), wired inline via the plugin's `hooks` block — Codex uses the same event schema and the same `SessionStart` event. Codex sets `PLUGIN_ROOT`/`PLUGIN_DATA` (and the `CLAUDE_*` pair for compatibility), which the script reads. See the caveat below. |
...
and the all-in-one [`.claude-plugin/plugin.json`](https://github.com/apache/magpie/blob/main/.claude-plugin/plugin.json)
