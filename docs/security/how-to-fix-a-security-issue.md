...
Rendered page: https://magpie.apache.org/docs/security/how-to-fix-a-security-issue/

Source: https://github.com/apache/magpie/blob/main/docs/security/how-to-fix-a-security-issue.md
...
[`<project-config>/project.md`](/docs/security/<project-config>/project)
(adopters bootstrap from the
[`projects/_template/`](https://github.com/apache/magpie/tree/main/projects/_template/) scaffold).

The end-to-end 16-step lifecycle is in [`process.md`](/docs/security/process). This
...
3. **CVE allocation.**
   ...
   [`security-cve-allocate`](https://github.com/apache/magpie/blob/main/skills/security-cve-allocate/SKILL.md) skill to
...
6. **Continuous improvement.**
   ...
   model — declared in
   [`<project-config>/security-model.md`](/docs/security/<project-config>/security-model)
...
* **Keep the reporter informed at every status transition** — see
  the [*Keeping the reporter informed*](/docs/security/roles#keeping-the-reporter-informed)
...
* **Confidentiality first.** Tracker URLs and `#NNN` identifiers
  ...
  *contents* — comments, labels, rollup entries, body excerpts —
  ...
  See the
  [Confidentiality of the tracker repository](https://github.com/apache/magpie/blob/main/AGENTS.md#confidentiality-of-the-tracker-repository)
  section of `AGENTS.md` for the three-layer rule and the
  sharing-with-non-team-recipients pattern. The
  [threat model](/docs/security/threat-model) covers the adversaries (P1–P5) the
