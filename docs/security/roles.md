...
Rendered page: https://magpie.apache.org/docs/security/roles/

Source: https://github.com/apache/magpie/blob/main/docs/security/roles.md
...
in [`process.md`](/docs/security/process).
...
- **I am looking up a specific step or label.** Go straight to
  [Process reference](/docs/security/process#process-reference-the-16-steps) or
  [Label lifecycle](/docs/security/process#label-lifecycle).
...
[`<project-config>/canned-responses.md`](/docs/security/<project-config>/canned-responses) — consult it before drafting a
reply from scratch.
...
[`forwarder-routing-policy.md`](/docs/security/forwarder-routing-policy) for
...
lives in
[`AGENTS.md` — Confidentiality of the tracker repository](https://github.com/apache/magpie/blob/main/AGENTS.md#confidentiality-of-the-tracker-repository).
Read it before editing anything that might be seen outside the team.

The [threat model](/docs/security/threat-model) enumerates the trust boundaries
...
1. **`import new reports`** —
   ...
   [Step 2](/docs/security/process#step-2--import-the-report).
2. **`sync all`** —
...
3. **`allocate CVE for issue #N`** —
   [`security-cve-allocate`](https://github.com/apache/magpie/blob/main/skills/security-cve-allocate/SKILL.md) when a report has
   been assessed as valid. See [Step 6](/docs/security/process#step-6--allocate-the-cve).
...
canned-response templates from [`<project-config>/canned-responses.md`](/docs/security/<project-config>/canned-responses)
...
a blocker. See
[Step 5](/docs/security/process#step-5--land-the-validinvalid-consensus).

If discussion stalls for about 30 days, escalate to a broader audience per
[Step 4](/docs/security/process#step-4--escalate-stalled-discussions).

### Allocating the CVE

Use [`security-cve-allocate`](https://github.com/apache/magpie/blob/main/skills/security-cve-allocate/SKILL.md). The skill asks up
...
reconcile the rest of the tracker. See [Step 6](/docs/security/process#step-6--allocate-the-cve)
for the full detail.
...
- [`security-issue-sync`](https://github.com/apache/magpie/blob/main/skills/security-issue-sync/SKILL.md) —
  *"sync <issue-ref>"* or *"sync all"*. Surfaces stalled issues, missing
  ...
  proposal.
- [`security-cve-allocate`](https://github.com/apache/magpie/blob/main/skills/security-cve-allocate/SKILL.md) — *"allocate a CVE
  for <issue-ref>"*.
- [`generate-cve-json`](https://github.com/apache/magpie/blob/main/tools/cve-tool-vulnogram/generate-cve-json/SKILL.md) — to
  ...
  [`<project-config>/project.md`](https://github.com/apache/magpie/blob/main/<project-config>/project.md#cve-authority)
...
ownership. See [Step 7](/docs/security/process#step-7--self-assign-and-implement-the-fix).

### Attempting an automated fix

Before writing the fix by hand, consider letting the
[`security-issue-fix`](https://github.com/apache/magpie/blob/main/skills/security-issue-fix/SKILL.md) skill try
...
[Step 9](/docs/security/process#step-9--open-a-private-pr-exceptional-cases). If it refuses,
...
see [Step 8](/docs/security/process#step-8--open-a-public-pr-straightforward-cases) and the
confidentiality rules in
[`AGENTS.md`](https://github.com/apache/magpie/blob/main/AGENTS.md#confidentiality-of-the-tracker-repository).
...
[Step 9](/docs/security/process#step-9--open-a-private-pr-exceptional-cases).
...
[Step 11](/docs/security/process#step-11--pr-merged) and [Step 12](/docs/security/process#step-12--fix-released).
...
- [`security-issue-sync`](https://github.com/apache/magpie/blob/main/skills/security-issue-sync/SKILL.md) — to
...
[Step 12](/docs/security/process#step-12--fix-released).
...
3. **Stop.** Sync drives the rest at the archive-URL trigger
   ([Step 14](/docs/security/process#step-14--capture-the-public-advisory-url-and-close-out)).
...
[Step 15](/docs/security/process#step-15--rm-verifies-the-close-out-landed).
...
[Step 16](/docs/security/process#step-16--credit-corrections).
