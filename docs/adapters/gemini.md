...
Rendered page: https://magpie.apache.org/docs/adapters/gemini/

Source: https://github.com/apache/magpie/blob/main/docs/adapters/gemini.md
...
The integration follows the harness contract in [add-a-harness](/docs/adapters/add-a-harness)
and [RFC-AI-0004](/docs/rfcs/rfc-ai-0004).
...
Install the [Gemini extension](/docs/setup/marketplace-install#google-gemini-cli) through [setup](https://github.com/apache/magpie/blob/main/skills/setup/SKILL.md), or use the canonical `.agents/skills/` links with a pinned snapshot.
Both methods expose the same skill sources.
The framework's [`GEMINI.md`](https://github.com/apache/magpie/blob/main/GEMINI.md) imports `AGENTS.md`; adopters retain their own project instructions alongside that context.
...
Use the framework's [sandbox primitive versions](https://github.com/apache/magpie/blob/main/tools/agent-isolation/pinned-versions.toml) when installing bubblewrap.
...
3. **Register the guard.** Merge the [BeforeTool hook](https://github.com/apache/magpie/blob/main/tools/agent-guard/README.md#gemini-cli) using the framework path above.
...
The [committed settings](https://github.com/apache/magpie/blob/main/.gemini/settings.json) are the profile's source of truth; avoid maintaining a second settings template.
...
The [clean-environment wrapper](https://github.com/apache/magpie/blob/main/tools/agent-isolation/README.md#explicit-environment-opt-in) strips inherited credentials and non-essential configuration variables.
...
The [policy file](https://github.com/apache/magpie/blob/main/.gemini/policies/magpie.toml) defines the complete command and path lists.
...
These settings preserve [per-proposal confirmation](/docs/rfcs/rfc-ai-0004#principle-1--human-in-the-loop-on-every-state-change).
...
Every Gemini upgrade also requires the [native policy probe](https://github.com/apache/magpie/blob/main/tools/sandbox-lint/tests/integration/README.md) against that version: normal CI skips it and cannot establish precedence over upstream built-in rules.
...
The project `BeforeTool` hook passes shell commands to Magpie's shared [action guard](https://github.com/apache/magpie/blob/main/tools/agent-guard/README.md#gemini-cli).
...
For example, the [Apache Projects](https://github.com/apache/magpie/blob/main/tools/apache-projects/tool.md#1-install-the-mcp-server) and [PonyMail](https://github.com/apache/magpie/blob/main/tools/ponymail/tool.md#1-install-the-mcp-server) adapters provide compatible server commands.
...
[`tools/spec-loop/lib.sh`](https://github.com/apache/magpie/blob/main/tools/spec-loop/lib.sh) invokes the CLI with
...
See the [spec-loop guide](https://github.com/apache/magpie/blob/main/tools/spec-loop/README.md) for its operating
...
- **Validation scope:** native API tests cover policy decisions across four approval modes and interactive/headless execution.
  ...
  The pilot and live sandbox probes used bubblewrap 0.6.1, below the framework's 0.11.2 pin, so they do not establish validation with the [pinned primitives](https://github.com/apache/magpie/blob/main/tools/agent-isolation/pinned-versions.toml).
...
The optional [runtime integration tests](https://github.com/apache/magpie/blob/main/tools/sandbox-lint/tests/integration/README.md) additionally exercise Gemini's own policy engine and Linux sandbox using synthetic files, without a model or authentication.
Use them when changing policy semantics or validating a runtime upgrade; normal CI runs the static checks only.
Complete the shared [harness validation](/docs/adapters/add-a-harness#step-5--validate-the-full-wiring) before submitting adapter changes.
