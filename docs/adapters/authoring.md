...
Rendered page: https://magpie.apache.org/docs/adapters/authoring/

Source: https://github.com/apache/magpie/blob/main/docs/adapters/authoring.md
...
When Magpie ships no [adapter](/docs/vendor-neutrality#tool-adapters) for
your backend — a forge, a CNA tool, a chat system, a mail archive, a VCS,
or a whole [organization](https://github.com/apache/magpie/blob/main/organizations/README.md) profile — you
...
[registry](/docs/adapters/registry) is the index of what already exists.
...
| **License** | Apache-2.0 ([§17](https://github.com/apache/magpie/blob/main/PRINCIPLES.md#17-contributions-land-under-apache-license-20)) | yours | the author's |
| **Who reuses it** | every adopter on that backend | your project | anyone who wires it in |
| **Discovery** | shipped in-tree | committed in your repo | optionally listed in the [registry](/docs/adapters/registry) |
| **Install** | part of the snapshot | committed override | you wire it in deliberately — never auto-fetched ([§13](https://github.com/apache/magpie/blob/main/PRINCIPLES.md#13-snapshot-plus-override-never-vendored-copies)) |
...
[`docs/extending.md`](/docs/extending) for the same model applied to every
extension type.
...
1. **Find the contract.** Pick the `tools/<contract>/` whose capability
   you are implementing (e.g. [`tools/cve-tool/`](https://github.com/apache/magpie/tree/main/tools/cve-tool/)
   for a CNA tool, [`tools/mail-archive/`](https://github.com/apache/magpie/tree/main/tools/mail-archive/) for
...
3. **Declare the metadata** the validator requires (see
   [`tools/AGENTS.md`](https://github.com/apache/magpie/blob/main/tools/AGENTS.md)):
...
5. **Add an eval** ([§8](https://github.com/apache/magpie/blob/main/PRINCIPLES.md#8-eval-is-a-release-blocking-discipline))
   under `tools/skill-evals/evals/` so the adapter's behaviour is graded,
   not just demoed.

The [`write-skill`](https://github.com/apache/magpie/blob/main/skills/write-skill/SKILL.md) flow and
[`CONTRIBUTING.md`](https://github.com/apache/magpie/blob/main/CONTRIBUTING.md) walk the conventions in detail;
...
one, copy [`organizations/_template/`](https://github.com/apache/magpie/tree/main/organizations/_template/) to
...
- **Keeping it external?** Optionally add a row to the
  [registry](/docs/adapters/registry#community--external-adapters) so others can
